...
`;
            ...
            vi.mocked(fs.existsSync).mockReturnValue(true);
            const config = loadConfig('/fake/config.yaml');
            expect(config.database.api_key).toBe('sk-123456');
...
`;
    ...
    describe('createDefaultConfig', () => {
        it('应该创建默认配置文件', () => {
            ...
            expect(vi.mocked(fs.mkdirSync)).toHaveBeenCalledWith('/mocked', expect.objectContaining({ recursive: true }));
            expect(vi.mocked(fs.writeFileSync)).toHaveBeenCalledWith('/mocked/config.yaml', expect.stringContaining('llm:'));
        });
        it('应该包含所有必需的配置项', () => {
            vi.mocked(fs.existsSync).mockReturnValue(false);
            vi.mocked(fs.mkdirSync).mockReturnValue('/mocked/dir');
